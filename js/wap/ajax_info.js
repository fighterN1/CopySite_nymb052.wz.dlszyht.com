...
ajaxInfo.productList = function (type, username, channelId, big_id, sub_id, sort, style, uuid, manu_id, guige, pricenum, salenum) {
	username 	= $.trim(username);
	channelId 	= parseInt(channelId);
	type 		= parseInt(type);
	big_id 		= parseInt(big_id);
	...
	if (uuid) {
		...
		$.ajax({
			...
			async: false,
			cache: false,
			data: {"type":type, "username":username, "channel_id":channelId, "big_id":big_id, "sub_id":sub_id, "sort":sort, "style":style, "u_u_id":uuid, "page":ajaxInfo.page, "manu_id":manu_id, "pricenum":pricenum, 'sale_num':salenum},
			success: function(data) {
				if ($.trim(data) != 'fail') {
					var myObj = eval('('+data+')');
					if (myObj.end == 1) $("#listMore").remove();
		...
		$.ajax({
			...
			async: false,
			cache: false,
			data: {"type":type, "username":username, "channel_id":channelId, "big_id":big_id, "sub_id":sub_id, "sort":sort, "style":style, "page":ajaxInfo.page, "manu_id":manu_id, 'pricenum':pricenum, 'sale_num':salenum},
			success: function(data) {
				if ($.trim(data) != 'fail') {
					var myObj = eval('('+data+')');
					if (myObj.end == 1) $("#listMore").remove();
