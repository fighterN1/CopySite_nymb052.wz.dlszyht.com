...
var tmp_arr=Domain.match(/[\w][\w-]*\.(?:com\.cn|net\.cn|org\.cn|com|cn|co|net|org|gov|cc|biz|info|hn|xyz|hk|us|mobi|wang|me|so|top|win)(\/|$)/);
Domain=tmp_arr[0];
...
function add_reduce_num3(arithmetic){
	...
	$('#pro_total').html('<b>总计：</b><span class="newPrice"><em>￥'+ allMoney +'</em></span>');
}
	...
	$.post(
        ...
        function (data) {
			if (!data || data == '') {
				...
				$("#shop_price").html('<font class="noPrice">暂无价格</font>');
				$("#Real_price").val(0);
                $("#pro_stock").html(0);
            ...
            if (price <= 0) {
                show_parice = '<font class="noPrice">暂无价格</font>';
                $("#jifen").hide();
            } else {
                show_parice = '￥'+ price;
